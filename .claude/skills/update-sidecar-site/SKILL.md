...
description: Keep sidecar.top (the official standalone site linked from the Chrome Web Store listing) in sync with this repo's README, FEATURES.md, PRIVACY.md, and the welcome.js/wallets.js app & wallet catalogs, and deploy it. Use when PRIVACY.md, FEATURES.md, or the README's Features section changes, when welcome.js APPS or wallets.js WALLETS change, when the Chrome Web Store URL changes, when app icons/logo assets change, or when explicitly asked to update or deploy sidecar.top.
---
...
sidecar-site/
  public/                # web root (the rsync source)
    ...
    apps.php             # Nostr app directory — renders data/apps.json (synced from welcome.js)
    wallets.php          # NWC wallet directory — renders data/wallets.json (synced from wallets.js)
    ...
    data/                # apps.json + wallets.json — GENERATED by scripts/sync-catalog.mjs, don't hand-edit
    ...
    includes/catalog-grid.php  # shared filter-bar + card grid used by apps.php and wallets.php
    ...
    assets/img/apps/     # catalog icons the extension bundles locally — copied by sync-catalog.mjs
    assets/js/           # slideshow.js (homepage), qrious.min.js + support.js (support page),
                         # catalog.js (apps/wallets category filter)
  scripts/
    sync-catalog.mjs     # regenerates data/*.json from the extension repo's welcome.js/wallets.js
  deploy/
...
**apps.php and wallets.php must match the extension's welcome.js and
wallets.js.** These pages render `public/data/apps.json` / `wallets.json`,
which are GENERATED — never hand-edit the JSON or transcribe entries manually.
Whenever the `APPS` array in `welcome.js` or the `WALLETS` array in
`wallets.js` changes — and as a standing step of **every version release** —
regenerate and the pages follow:

```bash
cd /Users/daniel/GitHub/sidecar-site
node scripts/sync-catalog.mjs   # reads ../sidecar by default; pass the repo path to override
```

The script also copies any extension-bundled icons (relative `icon:` paths)
into `public/assets/img/apps/` and stamps the JSON with the extension version
...
Check `http://localhost:8011/`, `/features.php`, `/apps.php`, `/wallets.php`,
`/support.php`, and `/privacy.php` render with no PHP warnings and all images
resolve (the apps/wallets card counts should match the extension's welcome.js
and wallets.js entry counts).
